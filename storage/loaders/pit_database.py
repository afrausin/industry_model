...
class PITDatabase:
    ...
    def __init__(
        ...
        Args:
            db_path: Path to SQLite database file. Defaults to storage/database/pit_macro.db
            data_dir: Path to data directory with JSON files. Defaults to storage/raw/
        """
        if db_path is None:
            # Default location: storage/database/pit_macro.db
            self.db_path = Path(__file__).parent.parent / "database" / "pit_macro.db"
        else:
    ...
    def _init_db(self):
        ...
        with self._get_connection() as conn:
            ...
            # UNIQUE includes title to allow multiple documents per date (e.g., multiple speeches)
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS documents (
                    ...
                    url TEXT,
                    created_at TEXT DEFAULT CURRENT_TIMESTAMP,
                    UNIQUE(source, document_type, document_date, title)
                )
            """)
    ...
    def migrate_from_json(
        ...
        # Migrate NY Fed research posts
        nyfed_stats = self._migrate_nyfed_posts(verbose)
        stats["documents_migrated"] += nyfed_stats.get("documents", 0)
        stats["errors"].extend(nyfed_stats.get("errors", []))
        
        # Migrate NBER digest/working papers
        nber_docs_stats = self._migrate_nber_documents(verbose)
        stats["documents_migrated"] += nber_docs_stats.get("documents", 0)
        stats["errors"].extend(nber_docs_stats.get("errors", []))
        
        # Migrate PIIE content
        piie_stats = self._migrate_piie_content(verbose)
        stats["documents_migrated"] += piie_stats.get("documents", 0)
        stats["errors"].extend(piie_stats.get("errors", []))
        
        # Migrate Brookings content
        brookings_stats = self._migrate_brookings_content(verbose)
        stats["documents_migrated"] += brookings_stats.get("documents", 0)
        stats["errors"].extend(brookings_stats.get("errors", []))
    ...
    def _migrate_fmp_data(self, verbose: bool = True) -> Dict[str, int]:
        ...
        - Market Quotes (all indices with categorization from fmp_config)
        
        Symbol categories are defined in ingest/fmp/fmp_config.py.
        To add new symbols, update that file and re-run the migration.
        """
        stats = {"events": 0, "quotes": 0, "errors": []}
        
        fmp_dir = self.data_dir / "fmp"
        ...
        if quotes_file.exists():
            try:
                ...
                with self._get_connection() as conn:
                    ...
                    # Get category lookup from centralized config
                    try:
                        from ingest.fmp import get_category_lookup
                        categories = get_category_lookup()
                    except ImportError:
        ...
        historical_stats = self._migrate_fmp_historical(verbose=verbose)
        stats["historical_symbols"] = historical_stats.get("symbols", 0)
        stats["historical_prices"] = historical_stats.get("prices", 0)
        stats["errors"].extend(historical_stats.get("errors", []))
        
        return stats
    
    def _migrate_fmp_historical(self, verbose: bool = True) -> Dict[str, int]:
        """
        Migrate FMP historical price data.
        
        Loads daily OHLCV data from storage/raw/fmp/historical/*.json
        into the market_prices table for time series analysis.
        """
        stats = {"symbols": 0, "prices": 0, "errors": []}
        
        historical_dir = self.data_dir / "fmp" / "historical"
        
        if not historical_dir.exists():
            return stats
        
        if verbose:
            print(f"📈 Migrating FMP historical prices...")
        ...
        for file_path in historical_files:
            try:
                ...
                if not symbol or not historical:
                    continue
                        ...
                        try:
                            cursor.execute("""
                                ...
                                (symbol, price_date, open, high, low, close,
                                 volume, change, change_percent, vwap, category)
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                            """, (
                                symbol,
                                record.get("date", ""),
                ...
                stats["errors"].append(f"Historical {file_path.name}: {str(e)}")
        
        if verbose:
            print(f"   Migrated {stats['symbols']} symbols, {stats['prices']:,} price records")
        
        return stats
    
    def _migrate_nyfed_posts(self, verbose: bool = True) -> Dict[str, int]:
        """Migrate NY Fed Liberty Street research posts."""
        stats = {"documents": 0, "errors": []}
        
        posts_file = self.data_dir / "ny_fed" / "liberty_street_posts.json"
        
        if not posts_file.exists():
            return stats
        
        if verbose:
            print(f"📰 Migrating NY Fed research posts...")
        ...
        try:
            ...
            for post in data.get("posts", []):
                ...
                if not title or word_count < 50:
                    continue
                
                # Use download date as document date since posts don't have dates
                self.insert_document(
                    source="NY Fed",
                    document_type="Research Post",
                    document_date=download_date,
                    release_date=download_date,
                    title=title,
                    content=content[:50000],  # Truncate long content
            ...
            if verbose:
                print(f"   Migrated {stats['documents']} NY Fed posts")
                
        except Exception as e:
            stats["errors"].append(f"NY Fed Posts: {str(e)}")
        
        return stats
    
    def _migrate_nber_documents(self, verbose: bool = True) -> Dict[str, int]:
        """Migrate NBER digest and working papers."""
        stats = {"documents": 0, "errors": []}
        
        nber_dir = self.data_dir / "nber"
        
        if not nber_dir.exists():
            return stats
        
        if verbose:
            print(f"📚 Migrating NBER documents...")
        
        # NBER Digest
        digest_file = nber_dir / "digest.json"
        if digest_file.exists():
            try:
                ...
                for article in data.get("articles", []):
                    ...
                    url = article.get("url", "")
                    date_str = article.get("date", download_date)
                    ...
                    self.insert_document(
                        ...
                        document_date=date_str if date_str else download_date,
                        release_date=date_str if date_str else download_date,
                        title=title,
                ...
                stats["errors"].append(f"NBER Digest: {str(e)}")
        
        # NBER Working Papers
        papers_file = nber_dir / "working_papers.json"
        if papers_file.exists():
            try:
                ...
                for paper in data.get("papers", []):
                    ...
                    url = paper.get("url", "")
                    date_str = paper.get("date", download_date)
                    ...
                    self.insert_document(
                        ...
                        release_date=date_str if date_str else download_date,
                        title=title,
                        content=abstract[:50000] if abstract else "",
                ...
                stats["errors"].append(f"NBER Working Papers: {str(e)}")
        
        if verbose:
            print(f"   Migrated {stats['documents']} NBER documents")
        
        return stats
    
    def _migrate_piie_content(self, verbose: bool = True) -> Dict[str, int]:
        """Migrate PIIE blog posts and charts."""
        stats = {"documents": 0, "errors": []}
        
        piie_dir = self.data_dir / "piie"
        
        if not piie_dir.exists():
            return stats
        
        if verbose:
            print(f"📊 Migrating PIIE content...")
        
        # PIIE Blog
        blog_file = piie_dir / "blog.json"
        if blog_file.exists():
            try:
                ...
                for post in data.get("posts", []):
                    ...
                    content = post.get("content", "")
                    url = post.get("url", "")
                    date_str = post.get("date", download_date)
                ...
                stats["errors"].append(f"PIIE Blog: {str(e)}")
        
        # PIIE Charts
        charts_file = piie_dir / "charts.json"
        if charts_file.exists():
            try:
                ...
                for chart in data.get("charts", []):
                    ...
                    url = chart.get("url", "")
                    date_str = chart.get("date", download_date)
                    ...
                    self.insert_document(
                        ...
                        release_date=date_str if date_str else download_date,
                        title=title,
                        content=description[:50000] if description else "",
                ...
                stats["errors"].append(f"PIIE Charts: {str(e)}")
        
        if verbose:
            print(f"   Migrated {stats['documents']} PIIE documents")
        
        return stats
    
    def _migrate_brookings_content(self, verbose: bool = True) -> Dict[str, int]:
        """Migrate Brookings research articles."""
        stats = {"documents": 0, "errors": []}
        
        brookings_dir = self.data_dir / "brookings"
        
        if not brookings_dir.exists():
            return stats
        
        if verbose:
            print(f"📚 Migrating Brookings content...")
        
        # Hutchins Center Research
        research_file = brookings_dir / "hutchins_research.json"
        if research_file.exists():
            try:
                ...
                stats["errors"].append(f"Brookings Research: {str(e)}")
        
        # Fiscal Impact Measure (FIM)
        fim_file = brookings_dir / "fim.json"
        if fim_file.exists():
            try:
                with open(fim_file, 'r') as f:
                    data = json.load(f)
                
                download_date = data.get("downloaded_at", "")[:10]
                title = data.get("title", "Hutchins Center Fiscal Impact Measure")
                ...
                if title:
                    self.insert_document(
                        source="Brookings",
                        document_type="Economic Indicator",
                        document_date=download_date,
                        release_date=download_date,
                        title=title,
                        content=content[:50000] if content else "",
                ...
                stats["errors"].append(f"Brookings FIM: {str(e)}")
        
        if verbose:
            print(f"   Migrated {stats['documents']} Brookings documents")
